...
function check_git() {
    if command -v git >/dev/null 2>&1; then
        ...
        sudo zypper --quiet install -y git > /dev/null 2>&1
...
function check_whiptail() {
    if command -v newt >/dev/null 2>&1; then
        ...
        sudo zypper --quiet install -y newt > /dev/null 2>&1
...
install_ansible() {
    sudo zypper --quiet install -y ansible > /dev/null 2>&1
...
install_nano() {
    sudo zypper --quiet install -y nano > /dev/null 2>&1
...
install_rclone() {
    sudo zypper --quiet install -y rclone > /dev/null 2>&1
...
install_vim() {
    sudo zypper --quiet install -y vim > /dev/null 2>&1
...
install_starship() {
    sudo zypper --quiet install -y starship > /dev/null 2>&1
...
function post_install() {
    if whiptail --title "Post Install" --yesno "Do you want to run user post install?" 10 80; then
        rm $HOME/.bashrc $HOME/.emacs $HOME/.inputrc $HOME/.profile $HOME/.zcompdump $HOME/.zsh_history> /dev/null 2>&1
